...
class Model:
    ...
    def train(self, epochs: int, learning_rate: float):
        ...
        for epoch in range(self.current_epoch, epochs):
            ...
            for i, word in enumerate(self.tokenizer.train):
                ...
                print(f"\rEpoch {epoch} | Word {i}/{len(self.tokenizer.train)} | Loss: {total_loss / (i + 1):.4f}", end="")
            self.save(loss=total_loss, learning_rate=learning_rate)
            print()

    def validate(self):
        """ Validates the model by predicting the top 5 most likely words """
        # range(len(valid_tk.words))
        for i in range(0, 100, 5):
            self.predict(i, "Valid")

    def predict(self, target_index: int, mode: str = "Train"):
        """ Predicts the top 5 most likely words """
    ...
    def forward_pass(self, target_index: int, mode: str = "Train") -> tuple[np.ndarray, list[int], np.ndarray]:
        """
